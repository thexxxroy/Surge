...
|            | 不填 key(默认)                  | 填了 OMDb key          |
| ---------- | ------------------------------- | ---------------------- |
| 豆瓣评分   | ✅ 按英文原名 + 年份 + 类型匹配 | ✅ 按 IMDb ID 精确匹配 |
| IMDb 评分  | —                               | ✅                     |
| 需要做的事 | 装上即可                        | 免费申请一次           |

不填 key 时豆瓣走的是**启发式匹配**,绝大多数情况正确,但冷门片或同名片可能选错条目。
...
**只显示豆瓣,没有 IMDb** — 未填 key,或 key 未激活,或该片在 IMDb 上没有评分。

**只显示 IMDb,没有豆瓣** — 豆瓣被限流,参见上面的路由配置建议。

**豆瓣评分对不上号** — 未填 key 时的启发式匹配选错了条目。填入 OMDb key 可解决。
