 ...
 * 最后更新:2026-09-07
 * 作者:thexxxroy
 * 项目:https://github.com/thexxxroy/Surge
 * 说明与参数见 netflix-ratings.sgmodule
 */
...
function parseNetflixJsonLd(html) {
  ...
  const type = d['@type'] === 'Movie' ? 'movie' : d['@type'] === 'TVSeries' ? 'series' : null
...
function pickDoubanCandidate(candidates, want) {
  const wantKind = want && want.type === 'series' ? '电视剧' : want && want.type === 'movie' ? '电影' : null
  let best = null
  let bestScore = 0
...
function buildOmdbUrl(apiKey, meta) {
  let u = 'https://www.omdbapi.com/?apikey=' + encodeURIComponent(apiKey) + '&t=' + encodeURIComponent(meta.name)
...
async function resolveRatings(titleId, deps) {
  ...
  diag.omdb = !deps.variant || deps.variant === 'n' ? 'no-key' : !omdb ? 'no-match' : !omdb.rating ? 'no-rating' : 'ok'

  // 有 imdbID 就按 ID 精确匹配,否则按英文名 + 年份 + 类型启发式匹配。
  diag.path = omdb && omdb.imdbId ? 'by-id' : 'by-title'
  const douban = omdb && omdb.imdbId ? await deps.fetchDoubanById(omdb.imdbId) : await deps.fetchDoubanByTitle(meta)
  diag.douban = !douban ? 'no-match' : !douban.rating ? 'no-rating' : 'ok'

  const imdb = omdb && omdb.rating ? { rating: omdb.rating, votes: omdb.votes, url: 'https://www.imdb.com/title/' + omdb.imdbId + '/' } : null
...
async function serveRatings(titleId, store, deps, now, fresh) {
  ...
  pushed.evicted.forEach(function (k) {
    store.write('', k)
  })
...
function pageAgent(extractTitleId, VERSION) {
  ...
  function memSet(id, d) {
    try {
      sessionStorage.setItem('nfr:' + id, JSON.stringify(d))
    } catch (_) {}
  }
  ...
  function chit(kind, score, votes, href) {
    ...
    a.title = votes ? meta.label + ' ' + score + ' · ' + votes + ' 人评价' : meta.label + ' ' + score
  ...
  function paintPending(anchor, id) {
    ...
    if (!reduceMotion && p.el.animate) {
      try {
        p.el.animate([{ opacity: 1 }, { opacity: 0.45 }, { opacity: 1 }], { duration: 1400, iterations: Infinity })
      } catch (_) {}
    }
  }
...
function handleInject() {
  try {
    ...
    if (typeof body !== 'string' || !/<\/body\s*>/i.test(body) || body.indexOf('surge-nfr-agent') !== -1) {
      $done({})
      return
    }
    ...
    if (ctKey && !/text\/html/i.test($response.headers[ctKey])) {
      $done({})
      return
    }
    ...
    const code = '(' + pageAgent.toString() + ')(' + extractTitleId.toString() + ', ' + JSON.stringify(VERSION) + ');'
    const tag = '<script id="surge-nfr-agent" data-v="' + VERSION + '"' + nonce + '>' + code + '</script>'
...
function dispatch() {
  if (typeof $request !== 'undefined' && $request.url && $request.url.indexOf(API_PATH) !== -1) {
    handleApiRequest()
    return
  }
  handleInject()
